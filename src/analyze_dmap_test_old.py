...
def analyze_dmap_test(img):
    ...
    accentuated = cv2.convertScaleAbs(blackhat_img, alpha=6, beta=0) # TODO: Make adjustable
    ...
    depth_threshold = 20 # TODO: Make adjustable
    ...
    blackhat_bg = cv2.convertScaleAbs(blackhat_bg, alpha=4, beta=0)

    # FOR TESTING PURPOSES (TODO: REMOVE)
    cv2.imshow("accentuated -- blackhat bg", blackhat_bg)
    cv2.waitKey(0)

    _, blackhat_bg_binary = cv2.threshold(blackhat_bg, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)

    # FOR TESTING PURPOSES (TODO: REMOVE)
    cv2.imshow("accentuated -- blackhat bg -- binary", blackhat_bg_binary)
    cv2.waitKey(0)

    contours, _ = cv2.findContours(blackhat_bg_binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    mask = np.ones(blackhat_bg_binary.shape, dtype="uint8") * 255
    ...
    blackhat_bg_filtered_binary = cv2.bitwise_and(blackhat_bg_binary, blackhat_bg_binary, mask=mask)

    # FOR TESTING PURPOSES (TODO: REMOVE)
    cv2.imshow("accentuated -- blackhat bg -- binary filtered", blackhat_bg_filtered_binary)
    cv2.waitKey(0)
    ...
    # Erode the adaptive binary image
    kernel = cv2.getStructuringElement(cv2.MORPH_RECT, (2, 2))
    adaptive_binary = cv2.erode(adaptive_binary, kernel, iterations=1)

    # Apply morphological operations to remove noise and hailpad edges
    kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (2, 2))
    adaptive_binary_refined = cv2.morphologyEx(
        adaptive_binary, cv2.MORPH_OPEN, kernel, iterations=2)
    
    # FOR TESTING PURPOSES (TODO: REMOVE)
    cv2.imshow("adaptive binary refined", adaptive_binary_refined)
    cv2.waitKey(0)
    ...
    # Separate out shallow artifacts by depth
    depth_threshold = 15 # TODO: Make adjustable
    filtered_blackhat = np.where(
        accentuated_refined < depth_threshold, 0, accentuated_refined)
    ...
    # Accentuate shallow artifacts
    filtered_accentuated = cv2.convertScaleAbs(filtered_blackhat_bg, alpha=4, beta=0) # TODO: Make adjustable
    ...
    clahe_img = clahe.apply(accentuated_refined)
